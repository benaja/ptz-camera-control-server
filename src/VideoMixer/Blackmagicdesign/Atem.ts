...
export class Atem implements IVideoMixer {
  ...
  public changeInput(newInput: number): void {
    if (this.connection.connection) {
      this.connection.atem.changePreviewInput(
        newInput,
        this.config.mixEffectBlock
      );
    }
  }
  ...
  public toggleKey(key: number): void {
    if (
      ...
      this.connection.atem.state !== undefined
    ) {
      const meState =
        this.connection.atem.state.video.mixEffects[this.config.mixEffectBlock];
  ...
  private getAuxSelectionInState(
    state: AtemState,
    aux: number
  ): number | undefined {
