...
export class NodeGamepad extends EventEmitter {
  ...
  public start(): void {
    ...
    this.log(
      `Starting connection procedure to device:${JSON.stringify(
        this.toIDeviceSpec(this.config)
      )}`
    );
  ...
  private connect(): void {
    ...
    if (this.config.serialNumber) {
      deviceList = deviceList.filter(
        (d) => d.serialNumber === this.config.serialNumber
      );
    }
    ...
    if (deviceList.length < 1) {
      ...
      this._connectRetryTimeout = setTimeout(
        () => this.connect(),
        this._connectionRetryTimeoutInMs
      );
      return;
    }
  ...
  private processButton(data: number[], config: IButtonConfig) {
    ...
    const newState: boolean = evaluate(config.value, {
      value: data[config.pin],
    });
  ...
  private processState(config: IStatusConfig, data: number[]) {
    ...
    if (oldState !== newState) {
      this.emit(
        config.name + ":change",
        this.getStateName(config.states, newState)
      );
    }
