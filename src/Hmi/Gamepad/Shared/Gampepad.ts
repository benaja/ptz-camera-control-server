...
export abstract class Gamepad implements IHmi {
  private readonly _connectionEmitter: StrictEventEmitter<
    EventEmitter,
    IConnection
  > = new EventEmitter();
  ...
  private readonly specialFunctionDefault: {
    [key in EButtonDirection]?: ISpecialFunction;
  } = {};
  private readonly specialFunctionAlt: {
    [key in EButtonDirection]?: ISpecialFunction;
  } = {};
  private readonly specialFunctionAltLower: {
    [key in EButtonDirection]?: ISpecialFunction;
  } = {};
  ...
  constructor(
    ...
    this.parseSpecialFunctionConfig(
      config.specialFunction.default,
      (key, config) => {
    ...
    if (config.specialFunction.altLower) {
      this.parseSpecialFunctionConfig(
        config.specialFunction.altLower,
        (key, config) => {
          this.specialFunctionAltLower[key] =
            SpecialFunctionFactory.get(config);
        }
      );
    }
  ...
  private printConnectionMessage(
    ...
    this.log(
      `Selected input:${index} (${connection})${this.getOnAirMessage(onAir)}`
    );
  }
